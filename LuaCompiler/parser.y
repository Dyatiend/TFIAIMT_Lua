...
expr:                 NIL { $$ = create_nil_expr_node(); } // TODO возможно нужно че-то другое делать??
                    ...
                    | expr FLOOR_DIV expr { $$ = create_bin_expr_node(_FLOOR_DIV, $1, $3); }
                    ...
                    | expr CONCAT expr { $$ = create_bin_expr_node(_CONCAT, $1, $3); }
                    | expr '<' expr { $$ = create_bin_expr_node(LESS, $1, $3); }
                    | expr LE expr { $$ = create_bin_expr_node(_LE, $1, $3); }
                    | expr '>' expr { $$ = create_bin_expr_node(GREATER, $1, $3); }
                    | expr GE expr { $$ = create_bin_expr_node(_GE, $1, $3); }
                    | expr EQL expr { $$ = create_bin_expr_node(EQUAL, $1, $3); }
                    | expr NOT_EQL expr { $$ = create_bin_expr_node(NOT_EQUAL, $1, $3); }
                    | expr AND expr { $$ = create_bin_expr_node(LOG_AND, $1, $3); }
                    | expr OR expr { $$ = create_bin_expr_node(LOG_OR, $1, $3); }
                    | '-' expr %prec UNARY { $$ = create_unary_expr_Node(UNARY_MINUS, $2); }
                    | NOT expr %prec UNARY { $$ = create_unary_expr_Node(_NOT, $2); }
